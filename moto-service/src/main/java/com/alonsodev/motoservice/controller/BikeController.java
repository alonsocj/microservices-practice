package com.alonsodev.motoservice.controller;

import com.alonsodev.motoservice.entity.Bike;
import com.alonsodev.motoservice.service.BikeService;
...
public class BikeController {
    ...
    public ResponseEntity<List<Bike>> getBikesByUser(@PathVariable("userId") int userId){
        List<Bike> bikes = bikeService.getByUser(userId);
        if (bikes.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(bikes);
